...
type Mixed = Player | Ball;

class Game {
    req: number;
    keyPlayer1: { [key: string]: boolean };
    keyPlayer2: { [key: string]: boolean };
    players: Player[];
    ball: Ball;
    ...
    checkCollision(obj1: Mixed, obj2: Mixed) {
        return (
    ...
    move() {
        ...
        if (this.checkCollision(this.ball, this.players[0])) {
            this.ball.speedX *= -1;
            const centerPaddleY =
                (this.players[0].y + this.players[0].height) / 2;
        ...
        if (this.checkCollision(this.ball, this.players[1])) {
            this.ball.speedX *= -1;
            const centerPaddleY =
                (this.players[1].y + this.players[1].height) / 2;
            const ballY = this.ball.y;
